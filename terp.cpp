...
namespace basecode {
    ...
    void terp::dump_heap(uint64_t address, size_t size) {
        auto program_memory = basecode::hex_formatter::dump_to_string(reinterpret_cast<const void *>(_heap), size);
        fmt::print("{}\n", program_memory);
    }
    ...
    bool
    terp::get_operand_value(result &r, const instruction_t &instruction, uint8_t operand_index, double &value) const {
        switch (instruction.operands[operand_index].type) {
            ...
            case operand_types::register_status:
                r.add_message("B005", "integer registers cannot be used for floating point operands", true);
                break;
    ...
    bool
    terp::get_operand_value(result &r, const instruction_t &instruction, uint8_t operand_index, uint64_t &value) const {
        switch (instruction.operands[operand_index].type) {
            case operand_types::register_integer:
